...
Exposed SSH access & Restrict inbound \texttt{TCP/22} to VPN, bastion, or approved administration and backup sources; default deny public SSH. & Untrusted sources cannot reach \texttt{backup01} SSH directly, reducing attack surface before authentication occurs. \\
...
\textbf{Prevention: restrict SSH network exposure.} A backup host should not normally expose SSH to arbitrary external sources. \texttt{backup01} should be reachable only through a VPN, bastion host, or private management subnet, with firewall rules allowing \texttt{TCP/22} only from approved administrator or backup infrastructure addresses. This acts before authentication: the \texttt{203.0.113.89} pattern would either be blocked before password guessing or become a clear policy violation if any login appeared from outside the approved source set.
...
\begin{enumerate}
    ...
    \item and the same IP then obtains an accepted login to the privileged \texttt{backup} account (the only account that successfully authenticates from \texttt{203.0.113.89}; \texttt{root}, \texttt{admin}, and \texttt{deploy} appear only as failed/invalid targets);
